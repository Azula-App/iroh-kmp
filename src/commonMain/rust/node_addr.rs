...
use iroh::{EndpointAddr, PublicKey, RelayUrl, TransportAddr};
...
impl TryFrom<NodeAddr> for EndpointAddr {
    type Error = IrohError;

    fn try_from(addr: NodeAddr) -> Result<Self, Self::Error> {
        let id = PublicKey::from_str(&addr.node_id).map_err(IrohError::msg)?;
...
/// Decode the peer's structured [`NodeAddr`] from a shareable ticket, without dialing. The
/// structured counterpart of [`crate::endpoint::node_id_from_ticket`], for apps that want to
/// hand the addresses to their own transport rather than re-share the opaque ticket string.
#[uniffi::export]
pub fn node_addr_from_ticket(ticket: String) -> Result<NodeAddr, IrohError> {
    let t = EndpointTicket::from_str(&ticket).map_err(IrohError::msg)?;
    Ok(t.endpoint_addr().clone().into())
}

/// Re-encode a [`NodeAddr`] as a shareable ticket string, the inverse of
/// [`node_addr_from_ticket`].
#[uniffi::export]
pub fn ticket_from_node_addr(addr: NodeAddr) -> Result<String, IrohError> {
    let addr: EndpointAddr = addr.try_into()?;
...
mod tests {
    ...
    #[test]
    fn node_addr_endpoint_addr_roundtrip() {
        let node_id = iroh::SecretKey::from_bytes(&[9u8; 32]).public().to_string();
        let addr = NodeAddr {
        ...
        let endpoint_addr: EndpointAddr = addr.clone().try_into().expect("try_into");
        assert_eq!(endpoint_addr.id.to_string(), node_id);
        assert_eq!(endpoint_addr.relay_urls().count(), 1);
        assert_eq!(endpoint_addr.ip_addrs().count(), 2);

        let roundtripped: NodeAddr = endpoint_addr.into();
        assert_eq!(roundtripped.node_id, addr.node_id);
